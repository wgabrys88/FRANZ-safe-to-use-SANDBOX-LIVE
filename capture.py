...
BIGGER PICTURE:
    ...
    by execute.py). This list contains two kinds of actions:

        1. Canvas-changing actions (left_click, right_click, double_left_click,
           drag, type) — in sandbox mode, these produce persistent white
           drawings on a black BMP canvas. Each action type draws a distinct
           shape: circles for left_click and double_left_click, rectangles for
           right_click, lines for drag, bitmap text for type. These drawings
           accumulate across turns and are NEVER erased (unless sandbox_reset).

           The type() action draws text at the last left-click position with
           a small horizontal offset and a thin cursor caret. This creates a
           tight spatial coupling between click and type: the VLM observes
           over many iterations that text always appears where it last clicked,
           teaching it the "click sets cursor, type writes at cursor" pattern.
           If no prior click exists, type() is silently skipped and reported
           as not-applied so execute.py can give accurate feedback.

        2. Mark-only actions (screenshot) — these NEVER modify the persistent
           canvas. They exist solely to trigger a volatile (ephemeral) visual
           annotation on the screenshot copy. The screenshot() action stamps a
           large photographer-style datetime watermark ("CAPTURED 2025-06-14
           18:32:07") onto the image so the VLM can perceive the passage of
           time in its narrative.

    In both modes (sandbox and real), optional visual marks are drawn on a COPY
    of the image as numbered overlays so the VLM can see what was executed.
    Each action type has a visually distinct mark shape:
        - left_click:        filled red circle with number
        - right_click:       filled red diamond with number
        - double_left_click: double concentric red rings with number
        - drag:              red arrow from start to end with number
        - type:              red underline at text position with number
        - screenshot:        large datetime watermark (white on dark backdrop)
    Marks are NEVER persisted to the sandbox canvas.
...
SANDBOX TRANSPARENCY:
    ...
    that execute.py can reconcile and give the VLM accurate feedback.
    screenshot() is always excluded from "applied" since it never touches
    the canvas.

MARKS vs SANDBOX DRAWINGS:
    - Sandbox drawings (white): PERSISTENT — written to sandbox_canvas.bmp
    - Visual marks (numbered shapes, datetime watermark): EPHEMERAL —
      drawn on a copy, never saved.
...
FILE PIPELINE:
    INPUTS (stdin JSON from execute.py):
        actions: list[str]        — canonical action strings (executed + mark-only)
        width, height: int        — output screenshot dimensions (0 = screen size)
        marks: bool               — draw visual marks overlay
    ...
    OUTPUTS (stdout JSON to execute.py):
        screenshot_b64: str       — base64 PNG of the current view
...
# ---------------------------------------------------------------------------
# PNG encoding (minimal valid PNG: IHDR + IDAT + IEND)
# ---------------------------------------------------------------------------
...
def _sandbox_apply(
    ...
    for line in actions:
        ...
        # screenshot() must NEVER modify the persistent canvas.
        if name == "screenshot":
            continue
...
# ---------------------------------------------------------------------------
# Visual marks (ephemeral overlay, drawn on a COPY, never persisted)
# ---------------------------------------------------------------------------
        ...
        match name:
            ...
            case "right_click":
                ...
                if px is not None and py is not None and (abs(x - px) + abs(y - py) > 30):
                    cv.line(px, py, x, y, TRAIL_COLOR, 4)
                r = 32
                diamond: list[Point] = [(x, y - r), (x + r, y), (x, y + r), (x - r, y)]
                cv.fill_polygon(diamond, MARK_FILL)
                cv.line(x, y - r, x + r, y, MARK_OUTLINE, 3)
                cv.line(x + r, y, x, y + r, MARK_OUTLINE, 3)
                cv.line(x, y + r, x - r, y, MARK_OUTLINE, 3)
                cv.line(x - r, y, x, y - r, MARK_OUTLINE, 3)
                _render_number(cv, x, y, n, MARK_TEXT, BLACK, 3)
                px, py = x, y
                n += 1
            case "double_left_click":
                ...
                if px is not None and py is not None and (abs(x - px) + abs(y - py) > 30):
                    cv.line(px, py, x, y, TRAIL_COLOR, 4)
                cv.circle(x, y, 36, MARK_OUTLINE, False, 4)
                cv.circle(x, y, 24, MARK_OUTLINE, False, 4)
                cv.circle(x, y, 18, MARK_FILL, True, 3)
                _render_number(cv, x, y, n, MARK_TEXT, BLACK, 3)
                px, py = x, y
                n += 1
            case "drag":
            ...
            case "type":
                ...
                ux = px + 9
                uy = py + 10
                cv.line(ux, uy, ux + text_w, uy, MARK_FILL, 4)
                cv.line(ux, uy + 2, ux + text_w, uy + 2, MARK_OUTLINE, 2)
                _render_number(cv, ux + text_w + 12, uy - 4, n, MARK_TEXT, BLACK, 3)
                n += 1
            case "screenshot":
                ...
                char_w = 6 * scale
                char_h = 7 * scale
                text_w = len(stamp) * char_w
                tx = (w - text_w) // 2
                ty = int(h * 0.85)
                backdrop: Color = (0, 0, 0, 100)
                pad_x = 8
                pad_y = 6
                for yy in range(ty - pad_y, ty + char_h + pad_y):
                    for xx in range(tx - pad_x, tx + text_w + pad_x):
                        cv.put(xx, yy, backdrop)
                _draw_text(cv, tx, ty, stamp, MARK_TEXT, scale)
            case _:
                continue
