...
BIGGER PICTURE:
    This file is the ACTION EXECUTOR in the FRANZ pipeline. Each turn:
        ...
        2. This file scans every line of that text for function call syntax.
           Any line containing '(' that ends with ')' is treated as a
           potential action call and parsed via safe AST (no eval — only
           literal constants are accepted).
        3. Each potential call is classified into one of three categories:
             - executed: valid call, recognized function, allowed by toolset,
               successfully dispatched (physically or to sandbox)
             - malformed: the line looked like a call attempt but could not
               be parsed, had an unknown function name, or had wrong/missing
               arguments. An explicit error reason is included so the VLM
               can learn and fix the syntax.
             - ignored: valid call but gated (master execute=False, or
               per-tool disabled), or a special non-executing action like
               screenshot() or focus().
        4. The list of executed canonical actions (plus mark-only actions
           like screenshot()) is forwarded to capture.py, which produces a
           screenshot with optional visual marks.
...
SANDBOX TRANSPARENCY:
    ...
    of each action (white shapes on a black canvas). From the pipeline's
    perspective, the actions "happened" — the screenshot shows the result.
    ...
    this file moves it from executed to malformed with a clear error
    reason so the VLM feedback accurately reflects what is visible.

    Mark-only actions (screenshot()) are never subject to reconciliation —
    they exist only to trigger volatile annotation and are always reported
    as "ignored" to the VLM.

FILE PIPELINE:
    INPUTS (stdin JSON from main.py):
        raw: str                  — prior VLM output text (read-only, for parsing)
        tools: dict[str, bool]    — tool allowlist
        ...
        marks: bool               — draw visual marks on screenshot

    OUTPUTS (stdout JSON to main.py):
        executed: list[str]       — canonical actions that were accepted AND applied
        malformed: list[str]      — lines that failed, each with "-> ERROR: reason"
        ignored: list[str]        — valid calls that were gated or non-executing
...
# ---------------------------------------------------------------------------
# VLM text → action line extraction
# ---------------------------------------------------------------------------

def _extract_call_lines(raw: str) -> list[str]:
    """Extract lines that look like function calls from VLM output text.

    Primary: looks for an ACTIONS: header and collects lines below it.
    Fallback: if no header found, accepts any line that contains '(' and
    ends with ')'. This is the primary mechanism for free-form output.
    """
    out: list[str] = []
    section = ""
...
# ---------------------------------------------------------------------------
# Safe AST parsing of a single action line
# ---------------------------------------------------------------------------
...
def _diagnose_failure(line: str) -> str:
    """Produce a human-readable error reason for a failed parse.

    This helps the VLM learn correct syntax through explicit feedback.
    """
    s = line.strip()
    try:
    ...
    if not isinstance(node.func, ast.Name):
        return "not a simple function name"
    name = node.func.id
    if name not in KNOWN_FUNCTIONS:
        return f"unknown function '{name}'"
    # If we get here, the call parsed but _parse_call returned None,
    # which means non-literal args
    for a in node.args:
        if not isinstance(a, ast.Constant):
            return f"argument must be a literal value, got {type(a).__name__}"
    for kw in node.keywords:
        if kw.arg is None:
            return "**kwargs not supported"
        if not isinstance(kw.value, ast.Constant):
            return f"keyword '{kw.arg}' must be a literal value"
...
# ---------------------------------------------------------------------------
# Argument validation helpers (return error string or None)
# ---------------------------------------------------------------------------

def _validate_click_args(args: list[object], kwargs: dict[str, object], name: str) -> str | None:
    x = _arg_int(args, kwargs, 0, "x")
    y = _arg_int(args, kwargs, 1, "y")
    if x is None or y is None:
        return f"{name} requires two integer arguments: {name}(x, y)"
    return None


def _validate_drag_args(args: list[object], kwargs: dict[str, object]) -> str | None:
    x1 = _arg_int(args, kwargs, 0, "x1")
    y1 = _arg_int(args, kwargs, 1, "y1")
    x2 = _arg_int(args, kwargs, 2, "x2")
    y2 = _arg_int(args, kwargs, 3, "y2")
    if None in (x1, y1, x2, y2):
        return "drag requires four integer arguments: drag(x1, y1, x2, y2)"
    return None


def _validate_type_args(args: list[object], kwargs: dict[str, object]) -> str | None:
    t = _arg_str(args, kwargs, 0, "text")
    if t is None:
        return 'type requires one string argument: type("text")'
    return None
...
def _run_capture(
    ...
    canvas. On failure: returns ("", actions) as fallback.
    """
    payload = json.dumps(
        {
            "actions": actions,
...
def main() -> None:
    ...
    for line in _extract_call_lines(raw):
        parsed = _parse_call(line)
        if parsed is None:
            # Line looked like a call but failed to parse — give explicit feedback
            reason = _diagnose_failure(line)
            malformed.append(f"{line} -> ERROR: {reason}")
            continue
        name, args, kwargs = parsed
        canon = _canon(name, args, kwargs)

        # screenshot() and focus() are ignored (non-executing), never "executed"
        if name == "screenshot":
            wants_screenshot = True
            ignored.append(canon)
            mark_only.append(canon)
            continue
        if name == "focus":
            ignored.append(canon)
            continue
        ...
        try:
            match name:
                case "left_click":
                    err = _validate_click_args(args, kwargs, name)
                    if err:
                        malformed.append(f"{canon} -> ERROR: {err}")
                    else:
                        x = _arg_int(args, kwargs, 0, "x")
                        y = _arg_int(args, kwargs, 1, "y")
                        if physical_execute:
                            _do_left_click(int(x), int(y))  # type: ignore[arg-type]
                ...
                case "right_click":
                    ...
                    if err:
                        ...
                        y = _arg_int(args, kwargs, 1, "y")
                        if physical_execute:
                            _do_right_click(int(x), int(y))  # type: ignore[arg-type]
                        executed.append(canon)
                case "double_left_click":
                    ...
                    if err:
                        ...
                        y = _arg_int(args, kwargs, 1, "y")
                        if physical_execute:
                            _do_double_left_click(int(x), int(y))  # type: ignore[arg-type]
                        executed.append(canon)
                case "drag":
                    err = _validate_drag_args(args, kwargs)
                    if err:
                        malformed.append(f"{canon} -> ERROR: {err}")
                    else:
                        x1 = _arg_int(args, kwargs, 0, "x1")
                        ...
                        y2 = _arg_int(args, kwargs, 3, "y2")
                        if physical_execute:
                            _do_drag(int(x1), int(y1), int(x2), int(y2))  # type: ignore[arg-type]
                        executed.append(canon)
                case "type":
                    err = _validate_type_args(args, kwargs)
                    if err:
                        malformed.append(f"{canon} -> ERROR: {err}")
                    else:
                        t = _arg_str(args, kwargs, 0, "text")
            ...
            malformed.append(f"{canon} -> ERROR: execution failed: {e}")

    # --- Capture screenshot and reconcile executed vs actually-applied ---
    all_for_capture = executed + mark_only
    screenshot_b64, applied = _run_capture(
        all_for_capture, width, height, marks, sandbox, sandbox_reset
    )

    # In sandbox mode, capture.py reports which actions it actually rendered.
    # If an action was "executed" here but NOT applied by capture.py (e.g.
    # type() with no cursor position), move it to malformed with a clear
    # reason so the VLM understands why it didn't work.
    # Mark-only actions are never in 'executed', so no reconciliation needed.
    if sandbox:
        ...
        if not_applied:
            for a in not_applied:
                malformed.append(
                    f"{a} -> ERROR: action had no visible effect"
                    " (e.g. type() requires a prior left_click to set cursor position)"
                )
