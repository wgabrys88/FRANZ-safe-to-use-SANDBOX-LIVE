...
BIGGER PICTURE:
    FRANZ is a self-narrative, self-adaptive AI agent loop. Each turn:
        ...
        2. The story is sent to execute.py, which scans it for function call
           lines, optionally performs them (physically or in sandbox), and
           returns a screenshot plus categorized feedback (executed, malformed,
           ignored).
        3. This file builds a VLM request with THREE messages:
             [0] system  — bootstrap prompt (capabilities, rules, task)
             [1] user #1 — the EXACT prior VLM output text (SST, see below)
             [2] user #2 — executor feedback text + current screenshot image
        4. The VLM responds with free-form text containing narrative and
           action calls interleaved however it chooses.
        5. That raw response is stored VERBATIM as the new story for next turn.
        6. Repeat forever.

    The VLM's output has NO required format. Any line that parses as a
    valid function call (e.g. left_click(500, 300)) is treated as an
    action. Everything else is narrative — the machine's growing memory.
    Malformed call attempts receive explicit error feedback so the machine
    can learn the correct syntax through experimentation.

    The narrative naturally accumulates transferable knowledge: observations
    become rules, strategies become methods, and the growing story functions
    as both memory and expertise document. If the story were given to a
    fresh process as a system prompt, that process would inherit the
    accumulated skill and understanding.
...
SANDBOX SAFETY:
    When SANDBOX=True (the default), no physical input is ever sent to the
    OS. All actions are rendered onto a persistent BMP canvas file. The only
    files written are sandbox_canvas.bmp, sandbox_state.json, state.json,
    and optional debug dumps in the dump/ folder. The only network traffic
    is to the local LM Studio API at localhost:1234.

FILE PIPELINE:
    INPUTS:
        - state.story  — prior raw VLM output text (verbatim, from state.json)
        - execute.py   — returns: executed[], malformed[], ignored[],
                                  screenshot_b64
...
1. You read what you wrote last time. That text is your only memory.
2. You see a screenshot of the screen right now.
3. You see feedback: which actions worked, which had errors, which were skipped.

Then you write your next output. What you write now, you will read in step 1 next time.

YOUR TOOLS — write these as lines in your output and they will run:
  left_click(x, y)         — click at position. Creates a small dot on screen.
  right_click(x, y)        — right-click at position. Creates a small square on screen.
  double_left_click(x, y)  — double-click at position. Creates a dot on screen.
  drag(x1, y1, x2, y2)    — draw a line from (x1,y1) to (x2,y2).
  type("text")             — type text at the place where you last left-clicked.
  screenshot()             — take a fresh screenshot so you can see what changed.

All coordinates are 0 to 1000. Top-left is (0,0). Bottom-right is (1000,1000).
...
Write what you plan to do next. Then write the tool calls to do it.

When you discover something new about how a tool works, write it down as a rule. For example: "RULE: left_click creates a white dot at that position." These rules help you remember.

When you find a good method for drawing something, write it down as a step-by-step plan. For example: "METHOD FOR CIRCLES: use 8 drags going clockwise, each one turning about 45 degrees."
...
def _save_state(
    ...
    try:
        out = {
            ...
            "malformed": executor_result.get("malformed", []),
            "ignored": executor_result.get("ignored", []),
...
def _infer(screenshot_b64: str, prev_story: str, feedback: str) -> str:
    ...
    Message layout (SST guarantee):
        messages[0] — system prompt (bootstrap)
...
def main() -> None:
    ...
    if SANDBOX and not SANDBOX_CANVAS.is_file():
        _run_executor("", sandbox_reset=True)

    time.sleep(1.0)

    state = _load_state()

    # sandbox_reset=True only on the very first turn of a fresh run
    # (turn==0 means no prior state was loaded). After that, False —
    # so the canvas accumulates drawings across turns.
    ...
    while True:
        ...
        # Reset sandbox only on the first turn of a fresh run
        sandbox_reset = first_turn
        first_turn = False

        # Run executor: parses actions from prev_story, returns feedback + screenshot
        executor_result = _run_executor(prev_story, sandbox_reset)
        screenshot_b64 = str(executor_result.get("screenshot_b64", ""))
        ...
        if executed:
            feedback_parts.append("executed=" + json.dumps(executed))
        if malformed:
            feedback_parts.append("malformed=" + json.dumps(malformed))
        if ignored:
            feedback_parts.append("ignored=" + json.dumps(ignored))
        if not executed and not malformed and not ignored:
            feedback_parts.append("no actions found in your previous output")
        feedback = "\n".join(feedback_parts)
